...
//广播系统信息
function send_sys_info(msg, msg_type) {
    _render.webContents.send('sys-info', msg, msg_type);
}
...
function send_syslog(logs) {
    ...
    for(let r of logs) {
        if (r.catalog === 'system') {
            ...
            send_sys_info(r.value, 'log_' + r.kind);
...
function _start_tick() {
    ...
    let last_t = 0;
    _timer = setInterval(()=>{
        let next_t = last_t + (last_t+40*1000*1000);
        let logs = c_syslog.find({
            time: {$jgte: last_t, $jlt: next_t}
        });
